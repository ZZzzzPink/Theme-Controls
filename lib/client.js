window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const NS = 'settings.themeControls';
    const ZH = { nav: '主题与字体' };
    const EN = { nav: 'Theme and fonts' };
    ...
    function applyPlugin(ctx) {
      ctx.effect(() => ctx.locale.register(NS, { zh: ZH, en: EN }), 'dsh-theme-controls: i18n');
      styles();
      apply(read());
    ...
    return { apply: applyPlugin, inject: ['slots', 'locale'] };
  }
});
